...
export default function EditAutomation({ workspace, automation, templates, channels, events }) {
    ...
    const publish = (event) => {
        ...
        form.put(`/app/automations/${automation.id}/publish`, { preserveScroll: true });
    };
    ...
    return (
        <Layout title={automation.name} workspace={workspace}>
            <Link href="/app" className="text-sm text-slate-400 transition hover:text-white">← Back to workspace</Link>
            <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                ...
                {automation.status === 'active' && <button className={secondaryButtonClass} onClick={() => router.post(`/app/automations/${automation.id}/pause`)}>Pause automation</button>}
            </div>

            <form onSubmit={publish} className="mt-8 grid gap-6 lg:grid-cols-[1fr_300px]">
                <section className={panelClass}>
                    <div className="mb-6 h-[360px] overflow-hidden rounded-xl border border-white/10 bg-slate-950"><ReactFlow key={`${form.data.goal.enabled}:${form.data.goal.event_id}|${form.data.steps.map((step) => `${step.type}:${step.timeout_action ?? ''}`).join('|')}`} nodes={flowNodes} edges={flowEdges} fitView fitViewOptions={{ padding: 0.24 }} onNodeDragStop={reorderFromCanvas} minZoom={0.35} maxZoom={1.5}><Background color="#334155" gap={18} /><Controls /></ReactFlow></div>
                    ...
                    {form.data.steps.map((step, index) => (
                        <div key={index}>
                            <div className="rounded-xl border border-white/10 bg-slate-900/80 p-5">
                                <div className="mb-4 flex items-center justify-between"><div><span className="mr-2 text-xs text-slate-500">{index + 1}</span><span className="font-semibold">{step.type.replaceAll('_', ' ')}</span></div><div className="flex gap-1"><TinyButton onClick={() => moveStep(index, -1)} disabled={index === 0}>↑</TinyButton><TinyButton onClick={() => moveStep(index, 1)} disabled={index === form.data.steps.length - 1}>↓</TinyButton><TinyButton onClick={() => removeStep(index)}>Remove</TinyButton></div></div>
...
function buildFlow(steps, triggerName, events, goal) {
    ...
    steps.forEach((step, index) => {
        ...
        const label = step.type === 'wait_for_event' ? `${index + 1}. Wait · ${eventName ?? 'event'}` : `${index + 1}. ${step.type.replaceAll('_', ' ')}`;
        nodes.push({ id: `step-${index}`, position: { x: 60, y }, data: { label }, style: step.type === 'wait_for_event' ? { ...mainStyle, border: '1px solid #38bdf8', background: '#082f49' } : mainStyle });
        y += step.type === 'wait_for_event' && step.timeout_action !== 'continue' ? 150 : 100;
    });
